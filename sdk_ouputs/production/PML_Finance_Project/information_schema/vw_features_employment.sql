create view information_schema.vw_features_employment
...
FROM information_schema.vw_identifier_columns id
         LEFT JOIN information_schema.calc_employment_features() ef(isin, revenue_per_employee, profit_per_employee,
                                                                    ebitda_per_employee, assets_per_employee,
                                                                    fte_growth_1y_pct, fte_growth_3y_pct,
                                                                    workforce_stability) USING (isin)
         LEFT JOIN information_schema.calc_employment_dynamics() ed(isin, fte_growth_2y_pct, fte_acceleration,
                                                                    workforce_volatility, hiring_intensity,
                                                                    productivity_trend, headcount_vs_revenue,
                                                                    workforce_efficiency_gain, layoff_risk_flag,
                                                                    rapid_hiring_flag, sustainable_growth_flag)
                   USING (isin);

comment on view information_schema.vw_features_employment is 'Employment metrics including productivity, workforce trends, and efficiency.
    Source functions: calc_employment_features, calc_employment_dynamics';

alter table information_schema.vw_features_employment
    owner to postgres;
