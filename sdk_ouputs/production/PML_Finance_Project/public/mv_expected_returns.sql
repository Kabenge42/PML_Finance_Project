...
FROM vw_identifier_columns id
         ...
         LEFT JOIN calc_price_target_dynamics() ptd(isin, pt_momentum_1w, pt_momentum_1m, pt_momentum_3m,
                                                    ...
                                                    pt_vs_price_momentum, analyst_coverage_trend) ON id.isin = ptd.isin
...
create unique index idx_mv_expected_returns_isin
    on mv_expected_returns (isin);

create index idx_mv_expected_returns_ticker
    on mv_expected_returns (ticker);

create index idx_mv_expected_returns_sector
    on mv_expected_returns (sector);

create index idx_mv_expected_returns_industry
    on mv_expected_returns (industry);

create index idx_mv_expected_returns_sector_upside
    on mv_expected_returns (sector asc, upside_potential desc);
