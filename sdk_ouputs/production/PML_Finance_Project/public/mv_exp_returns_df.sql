...
FROM vw_identifier_columns id
         ...
         LEFT JOIN calc_dividend_features() df(isin, dividend_streak, dividend_yield_ltm, dividend_yield_ntm,
                   ...
                   ON id.isin = df.isin
         LEFT JOIN calc_dividend_timing() dt(isin, days_since_ex_date, days_to_payment, dividend_announced_flag,
                                             ex_date_approaching_flag, dividend_frequency_score, dividend_consistency,
                                             recent_dividend_change, dividend_yield_vs_5y_avg) ON id.isin = dt.isin
         ...
         LEFT JOIN calc_volatility_surface_features() vsf(isin, vol_1m, vol_3m, vol_6m, vol_1y, vol_term_spread_short,
         ...
         LEFT JOIN calc_tax_rate_features() txf(isin, effective_tax_rate_ltm, effective_tax_rate_fy,
                                                tax_rate_yoy_change, tax_rate_qoq_change, tax_rate_stability,
                                                low_tax_flag, tax_rate_trend_4q) ON id.isin = txf.isin
         LEFT JOIN calc_opex_temporal_features() otf(isin, opex_fq, opex_ltm, opex_fy, opex_qoq_growth, opex_yoy_growth,
                                                     opex_vs_revenue_trend, sga_qoq_growth, sga_yoy_growth,
                                                     operating_leverage_score) ON id.isin = otf.isin
         LEFT JOIN calc_asset_sale_features() asf(isin, gain_loss_on_sale_of_assets_ltm, asset_sale_frequency,
                                                  asset_sale_trend) ON id.isin = asf.isin
         LEFT JOIN calc_fcf_estimate_features() fcfe(isin, fcf_est_avg_fy1e, fcf_est_avg_fy2e, fcf_est_avg_fy3e,
                                                     fcf_est_avg_fy4e, fcf_est_avg_fy5e, fcf_est_cagr_5y, fcf_est_trend)
         ...
         LEFT JOIN calc_price_target_achievement_features() ptaf(isin, pt_achievement_1y, pt_accuracy_1y,
                                                                 ...
                                                                 pt_median_vs_mean_spread, pt_high_low_convergence_1y,
                                                                 analyst_count_stability) ON id.isin = ptaf.isin
         LEFT JOIN calc_size_liquidity_features() slf(isin, market_cap, market_cap_country_r, log_market_cap,
                                                      ...
                                                      daily_turnover_ratio, size_class, style_class, liquidity_score)
                   ON id.isin = slf.isin
