...
INSERT INTO pml_df (ticker, isin, name, description, trading_region, trading_country, trading_country_name, exchange,
                    ...
                    num_sell_ratings, num_no_opinion_ratings, market_cap_global_r, market_cap_global_sec_r,
                    market_cap_region_r, market_cap_region_sec_r, market_cap_country_r, market_cap_country_sec_r,
...
SELECT NULLIF(TRIM(s."Ticker"), '')                                                                 AS ticker,
       ...
       -- Market-cap percentile ranks (vendor: 0-100, 100 = LARGEST). Six nested
       -- scopes: global / region / country, each also sector-relative ("Sec R").
       -- COALESCE(..., 0) matches the ratings counters above: a missing rank reads
       -- as "smallest", which the >= :min_rank screens exclude. mv_pymc_kalman_pt
       -- turns each into a feat_mcap_*_r ratio via (100 - r) / 100, so ~0 = largest.
       COALESCE(text_to_numeric_safe(s."Market Cap (Global R)"), 0)::INT                            AS market_cap_global_r,
       COALESCE(text_to_numeric_safe(s."Market Cap (Global Sec R)"), 0)::INT                        AS market_cap_global_sec_r,
       COALESCE(text_to_numeric_safe(s."Market Cap (Region R)"), 0)::INT                            AS market_cap_region_r,
       COALESCE(text_to_numeric_safe(s."Market Cap (Region Sec R)"), 0)::INT                        AS market_cap_region_sec_r,
       COALESCE(text_to_numeric_safe(s."Market Cap (Country R)"), 0)::INT                           AS market_cap_country_r,
       COALESCE(text_to_numeric_safe(s."Market Cap (Country Sec R)"), 0)::INT                       AS market_cap_country_sec_r,
