create view information_schema.vw_features_unusual_items
...
FROM information_schema.vw_identifier_columns id
         LEFT JOIN information_schema.calc_unusual_items_features() uif(isin, other_unusual_items_ltm,
                                                                        impairment_goodwill_ltm, asset_writedown_ltm,
                                                                        restructuring_charges_ltm, total_unusual_items,
                                                                        unusual_items_to_revenue,
                                                                        unusual_items_to_ebitda, has_unusual_items_flag,
                                                                        earnings_quality_impact) USING (isin);

comment on view information_schema.vw_features_unusual_items is 'Non-recurring and unusual items analysis for earnings quality assessment.
    Source function: calc_unusual_items_features';

alter table information_schema.vw_features_unusual_items
    owner to postgres;
