...
WITH mv_map(mv_name, model_target) AS (VALUES ('mv_pymc_earnings_beat'::text, 'earnings_beat'::text),
                                              ...
                                              ('mv_pymc_kalman_pt_v2'::text, 'kalman_pt_v2'::text),
     ...
     mv_cols AS (SELECT mm.model_target,
                        a.attname::text AS feat_name
                 FROM mv_map                mm
     ...
     cat AS (SELECT vw_pymc_feature_catalogue.model_target,
             ...
             GROUP BY vw_pymc_feature_catalogue.model_target, vw_pymc_feature_catalogue.feature_alias)
...
SELECT c.model_target,
       ...
       0                               AS catalogue_rows,
       'PHANTOM_CATALOGUE_ALIAS'::text AS status
FROM cat                   c
