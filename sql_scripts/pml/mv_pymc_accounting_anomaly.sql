...
SELECT isin,
       ...
       pml.accruals_ratio(net_eps_basic_ltm * shrs_out, cfo_ltm,
                          enterprise_value)                                                             AS feat_accruals_ratio,
       pml.calc_change_ratio(gross_profit_margin_pct_ltm,
       ...
       pml.safe_divide(eps_adj_ltm - net_eps_basic_ltm,
                       NULLIF(net_eps_basic_ltm, 0::double precision))                                  AS feat_eps_adj_gap,
       pml.safe_divide(cfi_ltm, NULLIF(cfo_ltm, 0::double precision))                                   AS feat_cfi_to_cfo,
       pml.safe_divide(cff_ltm, NULLIF(cfo_ltm, 0::double precision))                                   AS feat_cff_to_cfo,
       pml.calc_change_ratio(shrs_out, shrs_out_neg1fy)                                                 AS feat_share_inflation_1y,
       pml.safe_divide(issuance_common_stock_ltm,
