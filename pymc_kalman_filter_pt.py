...
#
# Imported under their ORIGINAL private names on purpose: this was a move, not a
# rewrite, so all ~200 call sites below are untouched and any behavioural
# difference is a defect. `setup_plotting`, `export_section` and friends are
# re-exported from here too, because notebooks import them from this module.
#
# `kalman_shared` also performs the Matplotlib backend selection that used to sit
# above, with two guards added: it no-ops under an explicit MPLBACKEND and where
# no display exists, so the headless v2 CLI cannot trip over TkAgg.
from probabilistic_ml_model.visualizations.kalman_shared import (  # noqa: F401
    C_AXES_BG, C_HIGHLIGHT, C_PANEL_BG, C_REF,
    _ARVIZ_STYLE_CANDIDATES, _AZP_BACKEND_HEAVY, _AZP_BACKEND_LIGHT,
    _AZP_HEAVY_BACKEND_ENV, _DEFAULT_EXPORT_DPI, _DEFAULT_EXPORT_PNG_HEIGHT,
    _DEFAULT_EXPORT_PNG_SCALE, _DEFAULT_EXPORT_PNG_WIDTH,
    _DEFAULT_RESULTS_DIRNAME, _DIAG_FACET_COLS, _DIAG_ROW_PX,
    _EXPORT_DIR_ALIASES, _EXPORT_MISC_DIR, _EXPORT_SECTION_DIRS,
    _EXPORT_SLUG_MAXLEN, _ExportState, _FIG_WIDTH_DEFAULT_PX, _LEGEND_FONT_SIZE,
    _PLOTLY_TEMPLATE, _RANK_DIST_ROW_PX, _REF_BAND_ALPHA, _REF_LINE_KINDS,
    _REF_LINE_OPACITY,
    _add_binned_density, _add_ref_band, _add_ref_line, _apply_dark_template,
    _apply_mpl_dark, _autosize_plotly, _azp_backend, _azp_figure_kwargs,
    _binned_density_trace, _clean_section_dir, _decimate_frame,
    _degenerate_posterior_vars, _diag_figure_kwargs, _display_width_px,
    _ecdf_xy, _export_dir_for, _export_figure, _export_path,
    _facet_grid_height_px, _fmt_axis, _forest_height_px, _hex_to_rgba,
    _hover_pct, _hover_price, _hover_prob, _in_ipython_kernel, _kde_xy,
    _mpl_figsize, _mpl_figure_of, _n_facets, _next_stem, _plotly_band,
    _plotly_figure_of, _polish_facet_axes, _present_vars,
    _quiet_degenerate_density, _rank_dist_figure_kwargs, _render_plotly,
    _resolve_env_setting, _safe_show, _show_fig, _slugify, _write_plotly_figure,
    _xval, enable_artifact_export, export_section, get_export_state,
    reset_export_state, set_export_section, set_viz_config_resolver,
    setup_plotting,
)
...
# Hand the shared figure layer this module's live config accessor. Three helpers
# there need run settings -- `_display_width_px` (fig_width_px),
# `_decimate_frame` (random_seed) and `get_export_state` (results_dir) -- and
# before the 2026-08-24 extraction they called `get_run_config()` directly.
#
# The FUNCTION is installed, not its result. `_run_config` is a singleton that
# `main(config=...)` and `set_run_config()` replace at will; passing a snapshot
# would freeze the figure width and the artifact root at import time and
# silently ignore every later override, which is exactly the regression an
# extraction like this invites.
set_viz_config_resolver(get_run_config)
