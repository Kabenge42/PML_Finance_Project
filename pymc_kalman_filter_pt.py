...
def _show_fig(fig) -> None:
    """Surface a matplotlib :class:`~matplotlib.figure.Figure` in any front-end.

    ``Figure.show()`` is only valid on an *interactive* backend; under the
    non-interactive Agg backend of a plain ``python pymc_kalman_filter_pt.py``
    run it raises ``UserWarning: FigureCanvasAgg is non-interactive, and thus
    cannot be shown`` and renders nothing. To keep figures that are returned
    (rather than self-shown) -- e.g. the ``plot_kalman_forecast`` structural
    forecasts -- surfacing in both contexts, an inline / interactive-widget
    notebook front-end is routed through IPython ``display`` (then closed so the
    cell-end auto-display does not duplicate it), a GUI backend through
    ``Figure.show()``, and a headless Agg script through ``plt.show()`` -- a
    safe no-op there. Mirrors the ``plt.show()`` / ``pc.show()`` convention used
    elsewhere in this module.

    Parameters
    ----------
    fig
        The figure to surface (typically the first element of a
    ...
    if any(tag in backend for tag in ('inline', 'nbagg', 'ipympl', 'widget')):
        ...
        plt.close(fig)
        return
    if plt.isinteractive() and 'agg' not in backend:
        fig.show()
        return
    plt.show()
...
def run_prior_predictive(model: "pm.Model", panel: KalmanPanelInputs):
    ...
    with model:
        prior_idata = pm.sample_prior_predictive(
            draws=2000, var_names=var_names,
    ...
    axes[1].set_title('Prior achieve_prob (sigmoid risk-adj return)')
...
def run_posterior_predictive(model: "pm.Model", idata, panel: KalmanPanelInputs) -> None:
    ...
    try:
        pc_ppc = azp.plot_ppc_dist(
            ...
            kind="ecdf", num_samples=2500, backend="matplotlib",
        )
        pc_ppc.show()
...
def plot_fused_model_effects(idata, panel: KalmanPanelInputs) -> None:
    ...
    # (a) Systematic-risk (beta) discount.
    sc = axes[0, 0].scatter(er, rar, c=avg_beta, cmap='viridis', s=14, alpha=0.75)
    ...
    axes[0, 1].set_ylabel('achieve_prob (sigmoid risk-adj return)')
...
class RiskBook:
    ...
    analytics : pandas.DataFrame
        ...
        ``p_upside_pos``, ``band_width_pct``, ``exp_vol_pct``, ``cvar05_pct``,
        ``ret_vol_ratio``, ``tail_risk_pct``, ``starr`` and the normalised
        ``book_weight`` (0 for names outside the sized book).
...
def compute_cvar_aware_book(
    ...
    *expected-shortfall* (CVaR of the posterior upside draws) — then sizes a long
    ...
    ----------
    idata
        Fitted fused-panel inference object (kept for signature symmetry / future
        posterior reads; the screen draws come via ``screen``).
    panel
    ...
    p_long
        Minimum ``P(upside>0)`` for long eligibility (default 0.80).
    ...
    _book = nm[(nm['expected_upside_pct'] > 0) & (nm['p_upside_pos'] >= p_long)
...
def export_analytics(idata, panel: KalmanPanelInputs, screen: ScreenContext,
    ...
    * ``reward_to_cvar`` — the STARR ratio (expected upside / binding tail risk) the
    ...
    must carry the three ``cvar_*`` columns above (schema-managed externally).
    """
    model_df = panel.frame
...
def run_recommendations(idata, panel: KalmanPanelInputs, results: pd.DataFrame,
    ...
    * **§1–§7 — directional signals.** Group- and name-level upside verdicts plus a
      band-width / coverage caution list (unchanged).
    ...
    * **§10 — CVaR-aware sizing.** A long book sized on a reward-to-CVaR (STARR) ratio
    ...
    _ra_book = nm[(nm['expected_upside_pct'] > 0) & (nm['p_upside_pos'] >= _p_long)
    ...
    _tail_book = nm[(nm['expected_upside_pct'] > 0) & (nm['p_upside_pos'] >= _p_long)]
    if len(_tail_book):
