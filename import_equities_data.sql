...
SELECT NULLIF(TRIM(s."Ticker"), '')                                        AS ticker,
       ...
       CASE
           WHEN current_fiscal.fiscal_quarter IS NOT NULL
               THEN 'Q' || current_fiscal.fiscal_quarter || ' ' || current_fiscal.fiscal_year
           ELSE 'n/a'
           END                                                             AS current_fiscal_quarter,
       CASE
           WHEN parsed.fy_end_date IS NOT NULL AND report_fiscal.next_quarter_year IS NOT NULL
               THEN 'Q' || calculate_next_fiscal_quarter(
                   NULLIF(TRIM(s."Next Earnings"), '')::DATE,
                   NULLIF(TRIM(s."Income Statement Report Date"), '')::DATE,
                   parsed.fy_end_date,
                   report_fiscal.earnings_report_frequency
                           ) || ' ' || report_fiscal.next_quarter_year
           ELSE 'n/a'
           END                                                             AS next_fiscal_quarter,
       COALESCE(report_fiscal.next_earnings_report_type, 'n/a')            AS next_earnings_report,
       report_fiscal.reporting_interval                                    AS reporting_interval,
       COALESCE(report_fiscal.earnings_report_frequency, 'Quarterly')      AS earnings_report_frequency,
       ...
       calculate_next_income_statement_report_date(
               NULLIF(TRIM(s."Income Statement Report Date"), '')::DATE,
               COALESCE(report_fiscal.earnings_report_frequency, 'Quarterly')
